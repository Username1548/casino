...
class BjRepositoryImpl implements BjRepository {
  //todo maybe should use Right()??
  ...
  Future<Either<Failure, BjTable>> createTable(int bet, String token,
    ...
    if (response.isRight()) {
      ...
      if (table.dealerHand.cards.length > 1) {
        ...
        final trueResponse = await remoteDataSource.createTable(
            bet, token, username, password);
        return trueResponse;
      }
    }
  ...
  @override
  Future<Either<Failure, BjTable>> addCard(String token, String username,
  ...
  @override
  Future<Either<Failure, BjTable>> stand(String token, String username,
      String password) async {
  ...
  @override
  Future<Either<Failure, BjTable>> double(String token, String username,
      String password) async {
  ...
  @override
  Future<Either<Failure, BjTable>> getTable(String token, String username,
      String password) async {
    final response = await remoteDataSource.getTable(token, username, password);
    return response;
  }
  ...
  @override
  Future<Either<Failure, String>> delete(String token, String username,
      String password) async {
  ...
  @override
  Future<String> reloadToken(String username, String password) async {
    final response = await remoteDataSource.generateToken(username, password);

    return response.getOrElse( // todo isRight()?
            () => ""
    );
  }
}